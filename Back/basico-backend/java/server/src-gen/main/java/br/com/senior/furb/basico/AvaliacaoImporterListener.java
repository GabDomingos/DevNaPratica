...
public class AvaliacaoImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(AvaliacaoImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  BasicoConstants.Events.IMPORT_AVALIACAO_EVENT, new ImportAvaliacaoEventPayload(status));
		}
	}
