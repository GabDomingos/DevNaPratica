...
import static br.com.senior.furb.basico.ProfessorEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Professor.PageRequest;
import br.com.senior.furb.basico.Professor.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateProfessor;
import br.com.senior.furb.basico.UpdateProfessor;
import br.com.senior.furb.basico.DeleteProfessor;
import br.com.senior.furb.basico.RetrieveProfessor;
import br.com.senior.furb.basico.ListProfessor;
...
@Component("furb.basico.ProfessorHandler")
public class ProfessorHandler implements CreateProfessor, CreateMergeProfessor, UpdateProfessor, DeleteProfessor, 
		 UpdateMergeProfessor, CreateBulkProfessor, RetrieveProfessor, ListProfessor, 
		 ImportProfessor, ExportProfessor {
	static final Logger logger = LoggerFactory.getLogger(ProfessorHandler.class);

	@Inject
	ProfessorCrudService service;
	...
	public Professor createProfessor(Professor toCreate) {
		try {
			ProfessorEntity entity = service.createProfessor(dtoConverter.toEntity(toCreate, ProfessorEntity.class));
			return dtoConverter.toDTO(entity, Professor.class);
	...
	@Transactional
	@Override
	public Professor createMergeProfessor(Professor toCreate) {
		try {
			ProfessorEntity entity = service.createProfessor(dtoConverter.toEntity(toCreate, ProfessorEntity.class));
			return dtoConverter.toDTO(entity, Professor.class);
	...
	public Professor updateProfessor(Professor toUpdate) {
		try {
			ProfessorEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Professor.class);
	...
	@Transactional
	@Override
	public Professor updateMergeProfessor(Professor toUpdate) {
		try {
			ProfessorEntity entity = service.retrieveProfessor(new Professor.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateProfessor(entity);
			return dtoConverter.toDTO(entity, Professor.class);
	...
	private ProfessorEntity doUpdate(Professor toUpdate) {
		ProfessorEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Professor_notFound", "Professor not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private ProfessorEntity doUpdate(ProfessorEntity entity) {
		return service.updateProfessor(entity);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_DELETE)
	@Override
	public void deleteProfessor(Id id) {
		this.service.deleteProfessor(id);
	}
	
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_VIEW)
	@Transactional(readOnly = true)
	@Override
	public Professor retrieveProfessor(Id id) {
		ProfessorEntity entity = service.retrieveProfessor(id);
		return dtoConverter.toDTO(entity, Professor.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listProfessor(PageRequest pageRequest) {
		...
		Page<ProfessorEntity> pages = this.service.listProfessorPageable(skip, top, orderBy, filter);
		List<ProfessorEntity> entities = pages.getContent();
		List<Professor> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Professor.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkProfessorOutput createBulkProfessor(CreateBulkProfessorInput toCreate) {
		if (toCreate != null) {
			List<Professor> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<ProfessorEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, ProfessorEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkProfessorOutput();
	}
	...
	public ImportProfessorOutput importProfessor(ImportProfessorInput request) {
		...
		ImportProfessorOutput output = new ImportProfessorOutput(importer.getId());
		return output;
	}
	...
	public ExportProfessorOutput exportProfessor(ExportProfessorInput request) {
		 ...
		 */
		ExportProfessorOutput output = new ExportProfessorOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new ProfessorImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Professor.class.getName());
					importer.setCrudService(ProfessorHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("professor", "blob");
