/* Database: PostgreSql. Generation date: 2019-06-30 16:05:58:858 */
/* Entity Professor */
...
alter table professor_turmas add constraint fkdvzia6ipjtyrgosahqcqarusu789 foreign key (professor_id) references professor (id);
alter table professor_turmas add constraint fkyf6kx0qu6bt860qxbdujksywxoqb foreign key (turmas_id) references turma (id);
alter table aluno_notas add constraint fkqxqtgfzcmskfk0vlgx9ajiwgzboi foreign key (aluno_id) references aluno (id);
alter table aluno_notas add constraint fktbknebaupr7ojk8tmfo9nfjggoyv foreign key (notas_id) references avaliacao (id);
alter table turma_alunos add constraint fk3e0aznkqshisvczmblsjizvmiim0 foreign key (turma_id) references turma (id);
alter table turma_alunos add constraint fku6oe2chsvsx5m2kfnpt6mfomwj4h foreign key (alunos_id) references aluno (id);
