...
public class RetornaMediaImpl {
	...
	public RetornaMediaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(RetornaMediaInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.RETORNA_MEDIA, //
	...
	*/
	public RetornaMediaOutput retornaMedia(RetornaMediaInput input, long timeout) {
		...
		RetornaMediaOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), RetornaMediaOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<RetornaMediaOutput> retornaMediaRequest(RetornaMediaInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, RetornaMediaOutput.class);
	}
