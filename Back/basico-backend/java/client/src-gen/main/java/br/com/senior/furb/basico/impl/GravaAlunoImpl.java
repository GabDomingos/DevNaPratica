...
public class GravaAlunoImpl {
	...
	public GravaAlunoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(GravaAlunoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.GRAVA_ALUNO, //
	...
	*/
	public GravaAlunoOutput gravaAluno(GravaAlunoInput input, long timeout) {
		...
		GravaAlunoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), GravaAlunoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<GravaAlunoOutput> gravaAlunoRequest(GravaAlunoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, GravaAlunoOutput.class);
	}
